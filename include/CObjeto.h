...
public:
    ...
    CObjeto(int posx,int posy, SDL_Renderer* renderizador,OffscreenRenderer offRender, int retiraFundo=1,int opacidade=255){
        strcpy(nomeImagem,"");
        renderer = renderizador;
        ...
        bitmap = NULL;//SDL_CreateRGBSurfaceWithFormatFrom(surface->pixels,surface->w,surface->h,offRender->GetDepth(),surface->pitch,surface->format->format);
        if (!bitmap)
