...
public:
    ...
    static int CriaObjetoOffScreen(int posx,int posy,OffscreenRenderer offRenderer,int retiraFundo=1,int opacidade=255){
        int resp = numObjetos->RetiraLivre();
        objetos[resp] = new CObjeto(posx,posy,renderer,offRenderer,retiraFundo,opacidade);
        totalObjetos++;
        return resp;
    }
